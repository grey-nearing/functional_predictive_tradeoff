function [T,Hs,Hx] = transfer_entropy(Ys,Xt,Yt,Bx,By)
...
assert(isempty(find(isnan(Ys))));
assert(isempty(find(isnan(Xt))));
assert(isempty(find(isnan(Yt))));

assert(isempty(find(Ys<=-9990)));
assert(isempty(find(Xt<=-9990)));
assert(isempty(find(Yt<=-9990)));
