...
if any(T(:)<0) && any(T(:)>0)
 ...
 fname1 = strcat(fname(1:end-4),'_positive_',fname(end-3:end));
 fid = fopen(fname1,'w');
 ...
 fname1 = strcat(fname(1:end-4),'_negative_',fname(end-3:end));
 fid = fopen(fname1,'w');
 format=repmat('%s ',[1,D-Du+1]);
 format = strcat(format,'\n');
 for x = 1:D+1
...
for x = 1:D
 for y = Du+1:D
  Tstring{x+1,y-Du+1} = num2str(abs(T(x,y)));
 end
end
